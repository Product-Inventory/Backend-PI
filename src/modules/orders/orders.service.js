...
export class OrdersService {
  ...
  async create(payload, currentUser = null) {
    const normalizedFolio =
      normalizeFolio(payload.folio)
    
    const existingOrder =
      await ordersRepository.findByFolio(
        normalizedFolio
      )
    if (existingOrder) {
    ...
    const client =
      await clientsRepository.findById(
        payload.clienteId
      )
    if (!client) {
      const error = new Error(
  ...
  async confirm(
    ...
    for (const item of currentOrder.items) {
      ...
      if (!product) {
        const error = new Error(
          `Producto no encontrado: ${item.productNombre}`
        )
        error.statusCode = 404
        throw error
      }
